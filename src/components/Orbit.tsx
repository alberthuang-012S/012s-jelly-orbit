...
export function Orbit({ activeJelly, resolving, attackCount = 0 }: { activeJelly: JellyUnit | null; resolving: boolean; attackCount?: number }) {
  return (
    <div className={`orbit-scene ${resolving ? 'orbit-scene--resolving' : ''}`} aria-label="Jelly Orbit">
      ...
      <div className="orbit-caption">
        ...
        {resolving ? `ORBITING · ${attackCount} POP${attackCount === 1 ? '' : 'S'}` : 'JELLY ORBIT'}
      </div>
    </div>
  )
}
